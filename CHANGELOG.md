...
- `prepare_task`/`task prepare` now requires a confirmed `task_preparation` interview session (`interview start --session-type task_preparation --goal ...`) instead of fabricating an agent from the bare goal string. A new `interview/task_gaps.py` supplies the fixed, always-asked task-scoped gaps (scope boundary, completion outcome, write permission, and — when `base_agent_ids` are given — a reuse-vs-new-agent check); a new `generation/task_synthesis.py` turns the confirmed answers into a grounded `AgentDefinition` (scope, when-to-use/when-not-to-use, read/write mode, completion criteria). The proposal step can be driven by a human answering interactively or by Claude running a self-interview over the same `start_project_interview` -> `get_next_questions` -> `submit_interview_answers` -> `confirm_interview_decisions` tool flow — no new MCP tools were needed for this.
