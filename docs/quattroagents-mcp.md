...
Run these commands from the repository you want QuattroAgents to manage. They store an absolute project path so the server does not depend on a later shell working directory. Use a published release tag to select a reproducible version; replace `v0.5.1` with a later tag or a commit SHA when required.
