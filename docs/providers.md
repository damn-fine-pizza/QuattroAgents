...
`scripts/setup.sh` optionally detects installed `rtk` and `codebase-memory-mcp` commands. Detection is re-runnable, makes no installation or MCP configuration changes, and reports a missing command without failing setup. `qagents doctor --json` reports the executable availability; it does not assert that an MCP server is configured or reachable.
